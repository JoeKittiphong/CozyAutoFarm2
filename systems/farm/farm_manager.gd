...
func complete_till(cell: Vector2i) -> void:
	if _farm_data.has(cell) and get_tile_state(cell) == TileState.BLUEPRINT:
...
func complete_water(cell: Vector2i) -> void:
	...
	if _farm_data.has(cell) and get_tile_state(cell) == TileState.PLANTED:
		if _farm_data[cell] is Dictionary:
...
func complete_harvest(cell: Vector2i) -> void:
	if _farm_data.has(cell) and get_tile_state(cell) == TileState.READY_TO_HARVEST:
		if _farm_data[cell] is Dictionary:
			_farm_data[cell].state = TileState.BLUEPRINT # Auto Loop back to start while keeping type/level
		else:
...
func deliver_wheat_to_mill(cell: Vector2i) -> void:
	...
	if data.wheat >= 3:
		data.state = "GRINDING"
		var speed_mult = pow(1.5, data.level - 1)
		data.timer = GRIND_TIME / speed_mult
...
func deliver_to_bakery(cell: Vector2i, type: String) -> void:
	...
	if data.flour == 1 and data.egg == 1 and data.milk == 1:
		data.state = "BAKING"
		var speed_mult = pow(1.5, data.level - 1)
		data.timer = BAKE_TIME / speed_mult
...
func get_tile_level(cell: Vector2i) -> int:
	if _farm_data.has(cell) and _farm_data[cell] is Dictionary:
		return _farm_data[cell].get("level", 1)
	# Check specialty building data if not in farm_data as dict
	if _bakery_data.has(cell): return _bakery_data[cell].level
	if _mill_data.has(cell): return _mill_data[cell].level
	return 1
...
func get_tile_upgrade_price(cell: Vector2i) -> int:
	...
	var state = get_tile_state(cell)
	var base = 25
	if state == TileState.BAKERY: base = 100
	elif state == TileState.MILL: base = 50
	return int(floor(base * pow(2.0, lvl - 1)))

func upgrade_tile(cell: Vector2i) -> bool:
	var state = get_tile_state(cell)
	var data = null
	if state == TileState.BAKERY: data = _bakery_data.get(cell)
	elif state == TileState.MILL: data = _mill_data.get(cell)
	else: data = _farm_data.get(cell)
	
	if not data or not data is Dictionary: return false
